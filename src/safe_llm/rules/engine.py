...
Run: python -m safe_llm.rules.engine
"""
...
DEFAULT_RULES_PATH = HERE / "data" / "constitution.yml"
...
class Engine:
    def __init__(self, rules: list[dict[str, Any]] | None = None, path: Path | None = None) -> None:
        if rules is not None:
            ...
            self.rules_from_yaml(DEFAULT_RULES_PATH)

    def rules_from_yaml(self, rules_yaml: Path) -> None:
...
def write_report(payload: dict[str, object], outputs_dir: Path | None = None) -> Path:
    out_dir = outputs_dir or OUTPUTS
    out_dir.mkdir(parents=True, exist_ok=True)
    path = out_dir / "rules_report.json"
